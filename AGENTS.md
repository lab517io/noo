...
- **Font Settings**: Separate user-configurable fonts for content (tree + editor: family, size, bold, italic) and UI chrome (menus/dialogs/toolbars: family + size scale factor)
...
- `pendingChangesCountProvider`: Count of unsyncedlocal changes

**MCP Providers** (`lib/presentation/providers/mcp_provider.dart`):
- `mcpConfigProvider`: The MCP preferences the server reacts to, as one `Equatable` value. Equality matters: settings are rewritten on every preference edit, and a config without `==` would report a change each time the user nudged the font size — which for a *fixed* port means a rebind that intermittently fails on a socket still in TIME_WAIT. (`SyncConfig` has no `==` and does exactly that to the LAN coordinator.)
...
- `settingsProvider`: Full application settings including:
  ...
  - `fontFamily`: Content font family (null = system default)
  - `fontSize`: Content font size in points, clamped [8, 48], default 14
  - `fontBold`: Whether content text is rendered bold
  - `fontItalic`: Whether content text is rendered italic
...
- File operations (New, Open, Export to Obsidian..., Import from Obsidian..., Sync Now [Ctrl+Shift+S], Sync Settings, Preferences, Exit)
- View options (Theme, Search)
- Tools (Timeline..., Time Report..., Compact Database... — runs `compactTaskHistoryOldValues` then `VACUUM` behind a modal barrier and reports the space returned to disk)
- Help and About
...
convention:
- An application menu named after the app holds About, Settings...,
  Registration..., the Services submenu, Hide/Hide Others/Show All and Quit —
  the items the other platforms keep in File and Help. Quit is a custom item,
...
Output: `build/noo-{version}-windows-x64.zip`
...
Work happens on the `dev` branch. What is published — `master` here,
`main` on GitHub — is **generated** from it: one squashed commit with no
history, rebuilt and force-pushed by `scripts/publish_public.py`, so anything
committed to it is discarded on the next publish. `docs/RELEASING.md` and the
publishing scripts live on `dev` only; no code does.
