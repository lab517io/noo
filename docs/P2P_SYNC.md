...
- **UDP broadcast** on port **46630**: broadcast probes
  ...
  would loop). Probes repeat every 15 s; peers age out after 60 s unheard.
...
A → B : proof_a = HMAC(key_peer, "init" | nonce_a | nonce_b | device_id_A),
        device_name_A (advisory, for B's prompt)        POST /auth/complete
B → A : session (opaque token, 10 min TTL)   — or 403, see §9.6
```
...
(§10.1) there is no next cycle, and one press would converge only one side.

`POST /peer/v2/sync-request {port}` closes the loop: after pulling, the
pressing device asks its peer to pull back.
...
- The return leg authenticates against the *requester*, which normally means
  a confirmation prompt (§9.6). The requester suppresses it for peers it has
  just sent a request to — see the grace window there.

### 9.6 Sync confirmation *[impl]*

Holding `key_peer` proves a device is ours. It does not establish that the
person in front of *this* device wants to sync right now, which is what
user-initiated sync (§10.1) is about. So the receiving side confirms.

The prompt is raised at the end of the handshake, **after** `proof_a` is
verified and **before** a session token exists:

- A stranger cannot raise a prompt. A bad proof is rejected first, so an
  unknown LAN host cannot make this device pop a dialog by connecting to it.
- A "no" answers **403** and issues no session. Since every data endpoint
  requires one, a decline leaks nothing — not even the vector.
- The prompt **denies itself after 45 s**. Nobody may be looking at the
  device, and the requester must not wait forever. Silence is not consent.
- No prompt is possible without a UI (app starting, no navigator): that also
  denies.
- `device_name` from `/auth/complete` names the requester in the prompt,
  falling back to the device id. It is advisory — outside the proof — but
  only a device already holding the key ever gets to state one, so the worst
  case is a device of ours mislabelling itself.

**One press, one prompt.** The return leg of §9.5 authenticates against the
device that pressed sync, whose user has already answered this exact
question. That device therefore records each peer it sends a sync-request to
and approves that peer's next handshake without asking, for a 2-minute grace
window — long enough to outlast the peer's own prompt, short enough that
consent does not linger. Every other inbound handshake prompts.

Sessions last 10 minutes but clients build a fresh one per exchange, so in
practice each press prompts once.

**Always allow.** The prompt offers "Always allow this device", which records
the peer's device id in the `lan_trusted_peers` property; a trusted peer's
handshake is approved without a prompt. This does *not* re-create automatic
sync — trust removes the **prompt**, never the **press**. Nothing on the LAN
exchanges until someone asks for it on one side or the other; a trusted phone
sitting next to a trusted desktop still does nothing at all.

- Trust is **per database file** and never synced. It records what the person
  using this device decided, which is not a fact about the data. It lives in
  the properties table beside the sync watermarks rather than in app settings,
  because a settings write rebuilds the sync config and would restart the LAN
  sockets — including during the exchange being approved.
- **Revocable** in Preferences → Sync, which lists every trusted device with
  its last announced name. "Always allow" is answered on whichever device
  happens to be nearby, so a way back is not optional.
- A **timeout never records trust**, whatever the checkbox says: walking away
  from the prompt is not agreeing forever.
- A corrupt stored value trusts nobody. Failing open would let anyone
  through; failing closed costs one prompt.
...
1. **Package** (local, no network): collect history rows above the v1
   ...
   (relay timer, manual relay sync, manual peer sync *[impl: peer discovery
   is no longer a trigger — see §10.1]*); an empty history delta packages
   nothing.
...
- **LAN**: ~~opportunistic and aggressive — sync on peer appearance and on a
  ...
  *[impl: **user-initiated only**. Discovery runs continuously and the UI
  shows which devices are nearby, but seeing a peer never starts an exchange.
  Data moves when the user presses "Sync with Nearby Devices" (File menu,
  Shift+F5, or the peer indicator in the status bar), the receiving device's
  user confirms it (§9.6), and the peer relays that press back via §9.5.
  Carrying a phone into the same room as a desktop must not move data on its
  own — being on the same network is not consent, and neither is holding the
  key.]*
