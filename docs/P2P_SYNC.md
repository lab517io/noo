...
*[impl: two refinements to the v1 rule.]*

- **Exact ties converge.** Two values for one field with the *same*
  timestamp are ordered by the values themselves: the greater string wins,
  `null` below every string, compared as the packager publishes them (a
  parent's world id, `blob:<hash>`, `"0"`/`"1"`). Both devices apply the
  same rule to the same pair, so exactly one of them changes and they end
  on the same value; under strict "newer wins" each kept its own and they
  stayed different for good. Logged as `lww-tie`, saying which side won.
- **A packet that fails to apply for this device's reasons is kept.** Only a
  fault of the packet's own content — a value that will not parse, an
  argument out of range — is written off as skipped (it would fail the same
  way every time). A database error or a closed connection rethrows: the
  transaction rolls back, the applied vector stays below the packet, the run
  fails with the cause, and the next run's backlog pass
  (`SyncService.applyBacklog`) applies it again. Previously any exception
  consumed the packet, so one transient SQLite error lost every change in
  it permanently.

Timeline entries restore like tasks and files: `removed = 0` clears the
flag (`undeleteTimeRecord`), history row included. *[impl: previously
logged "cannot be restored" and ignored, so a restore never left the
...
- A change skipped for a missing entity (or missing required owner — files,
  ...
  entity+field (compared as instants, since `toIso8601String` is not fixed
  width).
- **A parent not yet known is an orphan too** *[impl]*. A `parentId` change
  naming a task this device has not received is kept here rather than
  ignored, and a task *created* under an unknown parent is placed at the
  root provisionally with a `parentId` orphan recorded for it. Either way
  the move happens on the retry after the parent's creation arrives — from
  whichever stream carries it. "Task created on B, moved under it on A" is
  an ordinary cross-device race, and a move dropped at that moment was
  lost for good, since nothing re-sends it.
...
learns nothing but a random nonce. What such a host can do is make the
server *hold* things, so the server holds little *[impl]*: a request body
is read to at most 64 KiB (`PeerProtocol.maxBodyBytes` — every body on this
API is a small JSON object) and the rest left unread, which drops the
connection; and at most 64 handshakes wait on their `auth/complete` at once
(`maxPendingHandshakes`), the oldest making room for a new one. A real peer
completes within a round trip, so the one evicted is nearly always a stray. Subsequent requests carry
...
- **An attachment the relay refuses wedges its origin's stream.** A blob
  over `NOO_MAX_BLOB_SIZE` is refused with 413, the packet naming it cannot
  be stored without it, and §5.2 contiguity holds everything after that
  packet in the same stream. The client reports it *[impl]*: the run still
  completes (the pull is unaffected), the push stage shows the attachment by
  name and size with the relay's limit, `SyncResult.warning` carries the
  same line, and the blob is not re-uploaded during that session just to be
  refused again. A carried stream's refusal is only logged; its origin
  device reports it itself. There is no recovery yet short of *Reset device
  identity* after the bytes are gone from the database — a deleted row
  keeps its bytes until collection, and collection waits on the very upload
  that is blocked — so the useful fix is a client-side size check when the
  attachment is added, which is not in place. LAN peers are unaffected: the
  peer blob endpoint has no size limit.
