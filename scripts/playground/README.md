...
- LAN sync only happens while *Tools → Sync P2P...* is open: that dialog starts
  the peer server and discovery, syncs with every client that also has it open,
  and stops both when closed. The auto-sync interval drives the relay only. The
  scenarios therefore drive the app's own UI — **F5** for the relay, and for
  the LAN the menu item on *every* client, left open until the note has landed
  and then closed. (`Shift+F5` is the documented shortcut for Sync P2P, but the
  modifier does not survive `xdotool` here and the app sees a bare F5, i.e. a
  relay sync. The menu item is unambiguous.)
...
other two (their Sync P2P dialogs list each other). Verified directly — a probe
