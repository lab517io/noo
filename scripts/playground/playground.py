...
its keyring are its own. The three things the app would otherwise ask a person
for are pre-seeded:
  ...
  * LAN pairing — every client is pre-trusted by every other in the
    `lan_trusted_peers` property, so an exchange does not stop on the prompt.
...
class Playground:
    ...
    def _seed_databases(self):
        """Build each database with the app's own schema, and pre-trust the
        other clients so LAN exchanges never wait on the pairing prompt."""
        spec = {
            "clients": [
                {
                    "db": str(c.db),
...
"""
    ...
    def press_sync(self, c, nearby=False):
        """Press the client's own sync shortcut — F5 for the relay,
        Shift+F5 for nearby devices. LAN sync has no timer behind it: it
        ...
        if nearby:
            ...
            # sync. The menu item is unambiguous: "Sync with Nearby Devices"
            # is always the sixth entry of the File menu ("Sync Details..."
            # appears after it, so the offset does not move).
            self._xdo(["mousemove", "79", "105", "click", "1"])
            time.sleep(1.2)
            self._xdo(["mousemove", "160", "356", "click", "1"])
        else:
            self._xdo(["key", "F5"])
        # Both syncs end in a modal report the user closes, and a failed one
        # has Retry/Close buttons that Escape does not reach. Left open it
        # swallows the next keystroke, so it is clicked away — the dialog is
        # centred and its rightmost button sits here on this screen size.
        time.sleep(3.0)
        for _ in range(3):
    ...
    def converge(targets, predicate, nearby=False, rounds=3):
        ...
        for _ in range(rounds):
            for c in clients:
                ...
                time.sleep(1.0)
            if wait_for(predicate, timeout=sync_timeout // rounds, interval=2):
                return True
        return predicate()
