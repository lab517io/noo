...
class DockerPlayground(Playground):
    ...
    def press_sync(self, c, nearby=False):
        ...
        against that container's own X server."""
        ...
        if nearby:
            ...
            # menu item is unambiguous. "Sync with Nearby Devices" is always
            # the sixth entry of the File menu; "Sync Details..." appears
            # after it, so the offset does not move.
            xdo("mousemove", "79", "105", "click", "1")
            time.sleep(1.2)
            xdo("mousemove", "160", "356", "click", "1")
        else:
            xdo("key", "F5")
        # Both syncs end in a modal report, and a failed one has Retry/Close
        # buttons that Escape does not reach. Left open it swallows the next
        # keystroke, so it is clicked away: the dialog is centred, and its
        # rightmost button sits here on this screen size.
        time.sleep(3.0)
        for _ in range(3):
