...
import 'dialogs/peer_sync_request_dialog.dart';
...
class AppMenuBar extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // A sync populates per-stage state on its first run; before that there is
    // no progress report worth opening.
    final syncAttempted =
        ref.watch(syncProgressProvider).stageStates.isNotEmpty;
    // Nearby devices are worth naming in the menu item, so the user knows
    // whether pressing it can do anything before they press it.
    final peerCount = ref.watch(lanPeersProvider).value?.length ?? 0;
            ...
            MenuItemButton(
              ...
              child: Text(
                peerCount == 0
                    ...
                    : 'Sync with Nearby Devices ($peerCount)',
              ),
            ),
            // Only offer the detailed view once a sync has actually been
            // attempted this session — there is nothing to show before then.
            if (syncAttempted)
              MenuItemButton(
  ...
  static Future<void> syncNow(BuildContext context, WidgetRef ref) async {
    ...
    if (!service.hasRelay) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'No sync server configured. Use Sync with Nearby Devices '
  ...
  /// Exchange with the Noo devices currently visible on the LAN.
  ///
  /// Nothing on the LAN syncs on its own, so this (or the status bar's peer
  /// indicator, or the same action on the other device) is the only thing that
  /// moves data between two devices in the same room. Reports the outcome in a
  /// snackbar — unlike the relay path there is no staged progress to show.
  static Future<void> syncWithPeers(
    BuildContext context,
    WidgetRef ref,
  ) async {
    final messenger = ScaffoldMessenger.of(context);

    // The other device asks its user to confirm, so this can sit waiting for
    // someone to walk over and answer. Say so, rather than looking frozen.
    messenger.showSnackBar(const SnackBar(
      content: Text('Waiting for nearby devices to accept...'),
      duration: peerSyncRequestTimeout,
    ));

    final outcome = await runLanSync(ref);
    messenger.hideCurrentSnackBar();
    messenger.showSnackBar(SnackBar(content: Text(_peerSyncMessage(outcome))));
  }

  static String _peerSyncMessage(LanSyncOutcome outcome) {
    if (outcome.unavailable) {
      return 'Nearby sync is unavailable. Configure sync first.';
    }
    if (outcome.busy) return 'A sync with nearby devices is already running.';
    if (outcome.noPeers) return 'No Noo devices found on this network.';

    final parts = <String>[];
    if (outcome.peersSynced > 0) {
      parts.add('Synced with ${_devices(outcome.peersSynced)}');
      parts.add(outcome.changesApplied == 0
          ? 'already up to date'
          : '${outcome.changesApplied} '
              '${outcome.changesApplied == 1 ? 'change' : 'changes'} received');
    }
    // Worth its own wording: a decline (or an unanswered prompt) is someone
    // saying no, not something going wrong.
    if (outcome.peersDeclined > 0) {
      parts.add('${_devices(outcome.peersDeclined)} did not accept');
    }
    if (outcome.peersFailed > 0) {
      parts.add('${_devices(outcome.peersFailed)} could not be reached');
    }
    return '${parts.join(' — ')}.';
  }

  static String _devices(int count) =>
