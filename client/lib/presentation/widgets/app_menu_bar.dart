...
class AppMenuBar extends ConsumerWidget {
    ...
    final menuBar = MenuBar(
      children: [
        ...
        SubmenuButton(
          menuChildren: [
            MenuItemButton(
              onPressed: () => checkForUpdate(context, ref),
  ...
  static Future<void> newDatabase(BuildContext context, WidgetRef ref) async {
    ...
    // The password first, then the location. The save dialog writes an empty
    // file at the chosen path the moment it closes (see [pickSavePath]), so a
    // user who picks an existing database there and then cancels the password
    // dialog would be left with that database truncated to zero bytes. Asked
    // in this order, cancelling either dialog touches nothing.
  ...
  /// Help → Check for Update. Not offered on macOS, which has no published
  /// build to update to.
  static void checkForUpdate(BuildContext context, WidgetRef ref) {
    showDialog(
      context: context,
      builder: (context) =>
          UpdateDialog(onRestart: () => runExitSequence(ref)),
    );
  }
      ...
      builder: (context) => const AlertDialog(
        content: Row(
          children: [
            CircularProgressIndicator(),
            SizedBox(width: 16),
            Expanded(child: Text('Compacting database...')),
          ],
        ),
      ),
    );
