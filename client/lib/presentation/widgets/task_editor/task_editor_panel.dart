...
class _TaskEditorPanelState extends ConsumerState<TaskEditorPanel> {
  ...
  ///
  /// Shared by every instance of this State, because [dispose] fires a save it
  /// cannot await: a panel torn down and rebuilt at once — the layout crossing
  /// the phone threshold, the task screen popped and reopened — would read
  /// the database before that write landed, show the pre-write text, and
  /// autosave it over the real one. Entries are transient, removed as each
  /// write completes, so a swapped database cannot meet a stale one.
  static final Map<int, String?> _pendingSaves = {};
  ...
  Widget _buildTaskContent(BuildContext context, int selectedId) {
    ...
    return taskAsync.when(
      loading: () {
        final loaded = _loadedTaskId;
        if (loaded == null) {
          return const Center(child: CircularProgressIndicator());
        }
        // The same task being re-read (a save's invalidate, a sync refresh)
        // keeps its editor as it is. A *different* task keeps the previous
        // document on screen too, so a switch does not flash a spinner — but
        // frozen: keystrokes made now would land in the old document and be
        // wiped, unsaved, when the new one arrives.
        return _buildEditor(
          context,
          loaded,
          _editorContent,
          frozen: loaded != selectedId,
        );
      },
