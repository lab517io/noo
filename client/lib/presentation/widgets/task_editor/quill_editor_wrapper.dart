...
import 'word_navigation_actions.dart';
...
class QuillEditorWrapper extends ConsumerStatefulWidget {
  ...
  /// The document is shown but may not be edited for the moment.
  ///
  /// Set by the panel while it waits for another task's content: the previous
  /// document stays on screen rather than flashing a spinner, but anything
  /// typed into it would belong to no task — the load that follows replaces
  /// the document wholesale — so the keyboard is refused until it lands. Only
  /// the controller follows this; the toolbar keeps its buttons, since the
  /// wait is usually shorter than a frame and a toolbar that blinked on every
  /// task switch would be worse than the problem.
  final bool frozen;
...
class QuillEditorWrapperState extends ConsumerState<QuillEditorWrapper> {
  ...
  void _onFocusChanged() {
    ...
    // The phone toolbar follows focus — see [_showToolbar].
    if (mounted && isCompactLayout(context)) setState(() {});
  }

  /// Whether the formatting toolbar is shown.
  ///
  /// Always on a wide layout. On a phone it rides on top of the soft keyboard,
  /// and only while there is text being edited: a note being read has no use
  /// for forty pixels of buttons, and once the keyboard is up they are a
  /// tenth of what is left of the screen.
  ///
  /// "Being edited" includes a route that a toolbar button opened — the
  /// header, font and size menus, the colour and link dialogs. Those take
  /// focus with them, and the toolbar must outlive them (see [build]).
  bool _showToolbar(BuildContext context) {
    if (!isCompactLayout(context)) return true;
    if (_effectiveFocusNode.hasFocus) return true;
  ...
  /// Whether typing, pasting and the editing shortcuts are allowed right now.
  bool get _editable => !widget.readOnly && !widget.frozen;

  @override
  void didUpdateWidget(QuillEditorWrapper oldWidget) {
    super.didUpdateWidget(oldWidget);

    _controller.readOnly = !_editable;
  ...
  Widget _buildContextMenu(BuildContext context, QuillRawEditorState state) {
    ...
    // Our own two, after quill's. Both rewrite the selection, so neither has
    // anything to work on without one or in a read-only editor.
    if (!collapsed && !widget.readOnly) {
      buttonItems.addAll([
        ContextMenuButtonItem(
          label: 'Remove formatting',
          onPressed: () {
            state.hideToolbar();
            clearFormatting(_controller);
          },
        ),
        ContextMenuButtonItem(
          ...
          onPressed: () {
            ...
            formatSelectionAsTable(
              _controller,
              // A note already set in a console font keeps it.
              font: monospaceFontFor(ref.read(settingsProvider).fontFamily),
            );
          },
        ),
      ]);
  ...
  Widget build(BuildContext context) {
    ...
    // Hidden rather than removed when [_showToolbar] says so: a dropdown's
    // button awaits the menu it opened and applies the pick to the controller
    // afterwards, which it cannot do from a disposed State.
    final toolbar = Visibility(
      visible: _showToolbar(context),
      maintainState: true,
      child:
        ...
        TextFieldTapRegion(
          child: Container(
            decoration: BoxDecoration(
              ...
              // The edge that meets the text: below the toolbar on a desktop,
              // above it on a phone, where the toolbar sits under the text.
              border:
                  compact ? Border(top: divider) : Border(bottom: divider),
            ),
              ...
              config: QuillSimpleToolbarConfig(
                ...
                buttonOptions: QuillSimpleToolbarButtonOptions(
                  ...
                  fontFamily: QuillToolbarFontFamilyButtonOptions(
                    items: editorFontFamilies(
                        monospaceOnly: settings.monospaceFontsOnly),
    ...
    return Column(
      children: [
        // On a phone the toolbar goes at the bottom instead — see below.
        if (!compact) toolbar,
        ...
        Expanded(
          child: Padding(
            // Half on a phone, where 32dp of margin is text that does not fit.
            ...
            child: Listener(
              ...
              // Ctrl+Right in the last word of the document walks off the end
              // inside quill and comes back measured in the first line; this
              // stops it at the end of the text. See WordNavigationActions.
        ...
        // At the bottom on a phone, where the Scaffold's keyboard inset puts
        // it directly on top of the soft keyboard: under the thumb, beside the
        // keyboard's own row of suggestions.
        if (compact) toolbar,
      ],
    );
  }
