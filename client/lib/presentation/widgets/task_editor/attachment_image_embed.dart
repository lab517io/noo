...
class _AttachmentImageState extends ConsumerState<_AttachmentImage> {
  ...
  void didUpdateWidget(_AttachmentImage oldWidget) {
    ...
    if (imageSourceOf(oldWidget.node) != _source) _reload();
  }

  /// Build a fresh provider and learn the image's size again. Needed when the
  /// source changed, and when the bytes behind the same source did: a replace
  /// keeps the worldId, so neither the source nor this widget's key moves, and
  /// the mounted [Image] would keep painting the old frame — and the resize
  /// dialog would keep the old natural size — until the task was reopened.
  void _reload() {
    _provider = _buildProvider();
    _naturalSize = null;
    _listenForNaturalSize();
  ...
  Future<void> _showMenu(Offset globalPosition) async {
    ...
    switch (action) {
      ...
      case _ImageAction.replace:
        ...
        // An attachment-backed replace keeps the source, so nothing above
        // tells this widget the bytes moved; the provider revision did.
        if (mounted &&
            _provider is AttachmentImage &&
            (_provider as AttachmentImage).revision !=
                AttachmentImage.revisionOf(
                    (_provider as AttachmentImage).worldId)) {
          setState(_reload);
        }
