...
/// Most rows a result list builds. The lists are built eagerly (their rows
...
const int _kMaxListRows = 200;

/// The first [_kMaxListRows] of [items] as rows, then a line saying how many
/// were left out.
List<Widget> _capped<T>(
  BuildContext context,
  List<T> items,
  Widget Function(T item) row,
) {
  final shown = items.length > _kMaxListRows
      ? items.sublist(0, _kMaxListRows)
      : items;
  final hidden = items.length - shown.length;
  final theme = Theme.of(context);
  return [
    for (final item in shown) row(item),
    if (hidden > 0)
      Padding(
        padding: const EdgeInsets.only(top: 6),
        child: Text(
          '…and $hidden more — see Show Full Log',
...
class SyncProgressDialog extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final String? actionLabel = report.failed
        ? 'Retry'
        : idle || report.cancelled
            ? 'Sync Now'
            : null;

    // While running there are two ways out: Cancel stops the sync itself,
    // Hide only closes the dialog and leaves it running (the toolbar
    // indicator reports the outcome). Cancel stays disabled once pressed —
    // the run finishes the request already on the wire before it stops.
    final VoidCallback? onCancel =
        report.cancelling ? null : () => cancelSync(ref);
    final String cancelLabel = report.cancelling ? 'Cancelling…' : 'Cancel';
    ...
    return Column(
      ...
      children: [
                  ...
                  children: [
                    for (final stage in SyncStage.values) ...[
                      ...
                      _StageRow(
                        ...
                        fraction: report.fractionOf(stage),
        ...
        if (report.cancelled) ...[
          ...
          Text(
            'Stopped at your request. Everything synchronized up to that '
            'point is kept; the next sync continues from there.',
            style: theme.textTheme.bodySmall?.copyWith(color: theme.hintColor),
          ),
        ],
    ...
    return ConstrainedBox(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ..._capped(context, skips, (skip) => _LwwSkipRow(skip: skip)),
...
class _StageRow extends StatelessWidget {
  ...
  /// 0..1 while an active stage knows how far it is; draws a bar under the
  /// status.
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (fraction != null && state == SyncStageState.active) ...[
    ...
    return ConstrainedBox(
      ...
      child: SingleChildScrollView(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            ..._capped(context, changes, (change) => _ChangeRow(change: change)),
          ],
        ),
      ),
    );
