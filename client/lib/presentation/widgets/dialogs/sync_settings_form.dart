...
class _SyncSettingsFormState extends ConsumerState<SyncSettingsForm> {
    ...
    return Column(
      ...
      children: [
            ...
            children: [
              if (!relayConfigured)
                Padding(
                  padding: const EdgeInsets.only(bottom: 8),
                  child: Text(
                    'Scheduled sync needs a server. Devices on the same '
                    'network sync when you ask them to, from File \u2192 '
                    'Sync P2P...',
                    style: theme.textTheme.bodySmall
                        ?.copyWith(color: theme.colorScheme.outline),
                  ),
        ...
        ClassicGroupBox(
          title: 'Peer-to-peer',
          child: Text(
            'Devices on the same network that share your user name and '
            'database password can sync directly \u2014 no server needed. '
            'Open File \u2192 Sync P2P... on each of them: they find each '
            'other and sync while that window is open, and are not '
  ...
  Widget _buildRelayStorage(ThemeData theme, bool relayConfigured) {
    ...
    if (!relayConfigured) {
      return Text(
        'Nothing is stored on a server in this setup. Peer-to-peer sync '
