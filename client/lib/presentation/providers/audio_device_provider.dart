...
///
/// Takes the [ProviderContainer] rather than the button's `WidgetRef`: the
/// rescan is awaited, and a `WidgetRef` throws once its widget is gone — which
/// is where it is if Preferences was closed while the scan was still running.
/// The container outlives any dialog. The Refresh button is the only caller;
/// nothing inside a provider has a reason to re-enumerate on its own.
Future<void> refreshAudioDevices(ProviderContainer container) async {
  ...
  try {
    await container.read(audioDeviceSourceProvider).rescan();
  } on Object {
    // Ignored deliberately — see above.
  }
  container.invalidate(microphoneDevicesProvider);
  container.invalidate(speakerDevicesProvider);
}
