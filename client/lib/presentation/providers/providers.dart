...
/// Moves the selected task, for the app-level shortcuts in [MainScreen].
///
/// Published by the mounted task tree for the same reason as
/// [taskCreationProvider]: a moved task has to stay selected and be scrolled
/// back into view, which is the tree panel's state, not the controller's.
/// Null when no tree is on screen.
final taskMoveProvider =
    valueProvider<Future<void> Function(TaskMove move)?>(null);
