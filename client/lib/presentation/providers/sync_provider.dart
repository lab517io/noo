...
final syncApiClientProvider = Provider<SyncApiClient?>((ref) {
  ...
  // Closes the HTTP client's connections when the config changes; a run in
  // flight on the old client fails, which is the right answer to the server
  // or the account being changed underneath it.
  ref.onDispose(client.dispose);
  return client;
});

/// The sync log of the open database, or null when none is open. Shared by
/// the sync service, the LAN peer server and the Sync Log window.
final syncJournalProvider = Provider<SyncJournal?>((ref) {
  final db = ref.watch(databaseProvider);
  return db == null ? null : SyncJournal(db);
});
...
/// peer server and UDP discovery. Available whenever sync is configured and a
/// database is open; torn down (and rebuilt) when either changes.
///
/// Available is not running: the coordinator starts nothing on its own. Its
/// sockets exist only while the user has Sync P2P open
/// ([LanSyncCoordinator.startSession]), so outside that dialog the device is
/// neither discoverable nor reachable by peers. Independent of the relay
/// auto-sync timer — both paths feed the same packet store and deduplicate by
/// design.
...
class SyncProgressReport {
  ...
  /// Cancel was pressed and the run is heading for its next safe point.
  final bool cancelling;

  /// How far through each stage the run is (0..1), for the stages that know.
  final Map<SyncStage, double> stageFractions;
  ...
  double? fractionOf(SyncStage stage) => stageFractions[stage];

  bool get succeeded => result?.success ?? false;
  bool get cancelled => result?.cancelled ?? false;
  bool get failed => result != null && !result!.success && !cancelled;
  ...
  SyncProgressReport copyWith({
    ...
    String? errorMessage,
    bool? cancelling,
    Map<SyncStage, double>? stageFractions,
  }) {
    return SyncProgressReport(
      ...
      stageFractions: stageFractions ?? this.stageFractions,
    );
  }
}
...
class SyncProgressController extends Notifier<SyncProgressReport> {
  ...
  void update(SyncProgress progress) {
    ...
    // A bar belongs to a stage while it is running; once the stage settles
    // its status word says the rest.
    final fractions = Map<SyncStage, double>.of(state.stageFractions);
    if (progress.state == SyncStageState.active && progress.fraction != null) {
      fractions[progress.stage] = progress.fraction!;
    } else if (progress.state != SyncStageState.active) {
      fractions.remove(progress.stage);
    }
  ...
  /// Record that Cancel was pressed; the run itself reports when it stops.
  void markCancelling() {
    if (state.running) state = state.copyWith(cancelling: true);
  }

  /// Mark the run finished with its terminal [result].
  void finish(SyncResult result) {
    var states = state.stageStates;
    if (result.cancelled) {
      // Stages the run never reached did not happen, rather than still
      // waiting to.
      states = {
        for (final s in SyncStage.values)
          s: switch (state.stateOf(s)) {
            SyncStageState.pending ||
            SyncStageState.active => SyncStageState.skipped,
            final other => other,
          },
      };
    }
    state = state.copyWith(
      ...
      cancelling: false,
      stageStates: states,
      stageFractions: const {},
      result: result,
      // A cancel is not an error; there is nothing to put in the Error box.
      errorMessage:
          result.cancelled ? null : result.error ?? state.errorMessage,
    );
  }
}
...
/// Stop the relay sync in progress at its next safe point (see
/// [SyncService.requestCancel]). The dialog shows "Cancelling…" until the run
/// reports back through [runSyncWithProgress].
void cancelSync(WidgetRef ref) {
  final service = ref.read(syncServiceProvider);
  if (service == null) return;
  ref.read(syncProgressProvider.notifier).markCancelling();
...
Future<void> _runSync({
    ...
    progress.finish(
      SyncResult(
        ...
        error: syncService == null
            ...
            : 'No sync server is configured. Use "Sync P2P..." for local '
                'sync, or add a server in Preferences → Sync.',
      ),
    );
  ...
  // Guard against overlapping runs (auto-sync tick, double trigger). The
  // status is taken *before* the first await: a second caller arriving while
  // the editor flush below is pending would otherwise pass this check too,
  // reach performSync's own guard, and write its "already in progress" refusal
  // over the progress report of the run that is actually succeeding.
  ...
  // is a progress dialog that gets dismissed mid-run — nothing below may touch
  // `read` after an await (except the guarded invalidate).
  ...
  if (result.success) lastSync.value = DateTime.now();
  // A cancelled run may already have applied part of what it pulled.
  if (result.success || result.cancelled) {
    // Refresh views with remotely applied data so a stale editor doesn't
    // overwrite freshly pulled changes on the next auto-save.
