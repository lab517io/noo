...
class VoiceMemoNotifier extends Notifier<VoiceMemoState> {
  ...
  /// Which start the recorder belongs to. [start] spends its first moments
  /// awaiting a permission prompt and a device open with no recorder to
  /// cancel yet; a [cancel] in that window moves this on, and the start
  /// notices when it resumes and puts the device straight back.
  int _run = 0;
  ...
  VoiceMemoState build() {
    ...
    ref.listen<NooDatabase?>(databaseProvider, (previous, next) {
      if (next != null) return;
      if (_recorder != null || state.status == VoiceMemoStatus.starting) {
        unawaited(cancel());
      }
  ...
  Future<bool> start(int taskId) async {
    ...
    try {
      ...
      await recorder.dispose();
      // A start the user has already cancelled has nothing to report.
      if (run != _run) return false;
      return _fail('Could not start recording: $error');
    }

    if (run != _run) {
      // Cancelled while the device was opening — the workspace closed, or the
      // user gave up on the permission prompt. Dispose cancels first, so the
      // microphone that has just opened is closed again here.
      await recorder.dispose();
      return false;
    }
  ...
  Future<void> cancel() async {
    // Moved on even with no recorder yet: that is exactly the case of a start
    // still opening the device, and the bump is what tells it to stop.
    _run++;
    final recorder = _recorder;
    if (recorder == null) {
