...
class AudioCheckNotifier extends Notifier<AudioCheckState> {
  ...
  Future<void> start() async {
    ...
    if (run != _run) {
      // Cancelled while the device was opening. The panel is already idle and
      // ignores this run's ticks, so nothing would ever stop the recorder
      // otherwise: not the cap, and not Stop. Dispose cancels first.
      await recorder.dispose();
      return;
    }
  ...
  Future<void> finish() async {
    ...
    await _release();
    // A Cancel during the encoder tail has nothing to cancel — the recorder
    // has already stopped — and the player it stops is not playing yet. The
    // stamp is what keeps the clip from being played to a user who has
    // already cleared the panel.
    if (run != _run) return;
    ...
    try {
      final speaker = await selectedSpeaker(ref);
      if (run != _run) return;
      final played =
          await ref.read(memoPlayerProvider).play(memo.bytes, speaker: speaker);
      if (!played) {
