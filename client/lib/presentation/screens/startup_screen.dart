...
class _StartupScreenState extends ConsumerState<StartupScreen> {
  ...
  Future<void> _onWelcomePathChosen(String path,
      {bool? create, String? password}) async {
    final dbManager = ref.read(databaseManagerProvider);
    ...
    if (!mounted) return;
    await _handlePasswordRequired(dbManager,
        createNew: create, newPassword: password);
  }
  ...
  ///
  /// [newPassword] is a password the welcome screen already took for a new
  /// database, so the first creation attempt does not ask a second time; a
  /// failed attempt falls back to the dialog.
  Future<void> _handlePasswordRequired(DatabaseManager dbManager,
      {bool? createNew, String? newPassword}) async {
    ...
    if (!isNew && rememberPassword) {
      ...
      if (savedPassword != null) {
        ...
        if (biometricUnlock && !await BiometricGate.authenticate()) {
          ...
          // Whether the candidate came from the pre-per-database shared slot
          // is decided before the open, because opening changes nothing in
          // the keychain and a failure must leave that slot untouched — it may
          // be another database's password.
          final legacy = await secureStorage.isLegacyPassword(dbPath: path);
          success = await dbManager.openDatabase(path, password: savedPassword, saveAsDefault: saveAsDefault);
          if (success) {
            // Opening the file is the proof the legacy password is this
            // database's; only now is it filed under its own slot.
            if (legacy) await secureStorage.adoptLegacyPassword(dbPath: path);
            _navigateToMain();
            return;
          }
          // Saved password failed, clear it and prompt user. A legacy
          // candidate has no per-database slot to clear, and the shared one
          // is kept for the database it does open.
          if (!legacy) await secureStorage.deletePassword(dbPath: path);
    ...
    while (!success && mounted) {
      final PasswordDialogResult? passwordResult;
      if (isNew && newPassword != null) {
        passwordResult = PasswordDialogResult(password: newPassword);
        // Used once: a creation that fails with it goes back to the dialog.
        newPassword = null;
      } else if (isNew) {
        passwordResult =
            await PasswordDialog.showCreate(context, databasePath: path);
      } else {
        passwordResult = await PasswordDialog.showOpen(
          context,
