...
class _MainScreenState extends ConsumerState<MainScreen> {
  ...
  bool _handleGlobalKeyEvent(KeyEvent event) {
    ...
    // Shift+F5 is Tools → Sync P2P..., and this handler sees the key before
    // any binding could: without the check it ran a relay sync instead, and
    // the P2P dialog never opened.
    _triggerSyncShortcut(peers: keyboard.isShiftPressed);
    return true;
  }

  /// Run a manual sync from the F5 shortcut — the relay's, or with LAN peers
  /// for Shift+F5 — ignoring the press if the screen is gone or a sync
  /// triggered this way is already in flight.
  Future<void> _triggerSyncShortcut({bool peers = false}) async {
    if (!mounted || _syncShortcutBusy) return;
    _syncShortcutBusy = true;
    try {
      if (peers) {
        await AppMenuBar.syncWithPeers(context, ref);
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    return CallbackShortcuts(
      bindings: {
        ...
        // Moving the selected task, routed through the tree panel the same
        // way so the moved row stays in view. These fire with the note editor
        // focused — it is where focus lives while working in the tree — which
        // is why they are on a combination text editing can spare; see
        // treeMoveActivator.
        treeMoveActivator(LogicalKeyboardKey.arrowUp): () =>
            ref.read(taskMoveProvider)?.call(TaskMove.up),
        treeMoveActivator(LogicalKeyboardKey.arrowDown): () =>
            ref.read(taskMoveProvider)?.call(TaskMove.down),
        treeMoveActivator(LogicalKeyboardKey.arrowLeft): () =>
            ref.read(taskMoveProvider)?.call(TaskMove.toParentLevel),
        treeMoveActivator(LogicalKeyboardKey.arrowRight): () =>
            ref.read(taskMoveProvider)?.call(TaskMove.underPrevious),
  ...
  Widget _buildCompact(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        // The database name is reference, not a heading: most of the time it
        // is the only database there is. Small and muted, so the bar can be
        // lower and the room goes to the tree; long-press shows it in full.
        toolbarHeight: 48,
        titleSpacing: 16,
        title: Tooltip(
          message: fileName ?? AppConstants.appName,
          child: Text(
            fileName ?? AppConstants.appName,
            maxLines: 1,
        ...
        actions: [
          ...
          PopupMenuButton<_CompactMenuAction>(
            ...
            itemBuilder: (context) => [
              ...
              const PopupMenuItem(
                value: _CompactMenuAction.syncP2p,
                child: Text('Sync P2P...'),
              ),
              const PopupMenuItem(
              ...
              const PopupMenuItem(
                value: _CompactMenuAction.compactDatabase,
                child: Text('Compact Database...'),
              ),
              const PopupMenuDivider(),
              const PopupMenuItem(
              ...
              const PopupMenuItem(
                value: _CompactMenuAction.checkForUpdate,
  ...
  void _onCompactMenuAction(BuildContext context, _CompactMenuAction action) {
    switch (action) {
      ...
      case _CompactMenuAction.syncP2p:
        AppMenuBar.syncWithPeers(context, ref);
      ...
      case _CompactMenuAction.timeReport:
        AppMenuBar.showTimeReport(context);
      case _CompactMenuAction.compactDatabase:
        AppMenuBar.compactDatabase(context, ref);
      ...
      case _CompactMenuAction.preferences:
        AppMenuBar.showPreferences(context, ref);
      case _CompactMenuAction.checkForUpdate:
        AppMenuBar.checkForUpdate(context, ref);
