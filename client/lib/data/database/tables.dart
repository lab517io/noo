...
/// One sync run in the sync log: a relay sync, a LAN exchange this device
/// started, a peer pulling from this device, a compaction, and so on.
///
/// The sync log is **local diagnostics**: never synced, never history-tracked,
/// pruned to the last 30 days / 200 runs. It exists because a sync's decisions
/// — which packets arrived under which hash, which changes lost LWW and to
/// what, which became orphans or conflict copies — are otherwise gone the
/// moment the run ends, and "why does this device show something different"
/// can only be answered from them.
@DataClassName('SyncLogRunRow')
@TableIndex(name: 'idx_sync_log_runs_started', columns: {#startedAt})
class SyncLogRuns extends Table {
  IntColumn get id => integer().autoIncrement()();
  ...
  TextColumn get outcome => text().withDefault(const Constant('running'))();
...
/// One event of a [SyncLogRuns] run. Most columns are optional: a packet
/// event fills the packet columns, a change decision the entity columns.
@DataClassName('SyncLogEventRow')
@TableIndex(name: 'idx_sync_log_events_run', columns: {#runId, #seq})
@TableIndex(name: 'idx_sync_log_events_world', columns: {#worldId})
class SyncLogEvents extends Table {
  IntColumn get id => integer().autoIncrement()();
  ...
  /// in, out, or local.
  TextColumn get direction => text().withDefault(const Constant('local'))();

  TextColumn get originDevice => text().nullable()();
