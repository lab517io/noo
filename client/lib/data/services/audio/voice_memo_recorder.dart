...
class VoiceMemoRecorder {
  ...
  /// True while [start] is waiting for the engine to open the device.
  bool _starting = false;

  /// Set by [cancel] or [dispose] arriving during that wait. Nothing is
  /// recording yet at that point, so there is nothing to stop — but the device
  /// is about to open, and a start that then went on as if nothing had
  /// happened would leave the microphone live with no one holding it.
  ...
  /// Begin recording.
  ///
  /// Throws [StateError] when already recording, or when [cancel] or [dispose]
  /// arrived while the device was opening — the recording is then already
  /// discarded, and nothing is left running. Throws whatever the engine throws
  ...
  Future<void> start({va.AudioDevice? microphone}) async {
    if (_recording || _starting) throw StateError('already recording');
    ...
    if (_abandoned) {
      // A cancel came in while the device was opening. Close it again now that
      // it is open, and tell the caller its start did not take rather than
      // returning as though it had.
      try {
        await _engine.cancel();
      } on Object {
        // Best effort, as in [cancel].
      }
      throw StateError('cancelled while starting');
    }
