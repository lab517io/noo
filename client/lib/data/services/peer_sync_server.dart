...
class PeerProtocol {
  ...
  /// Most bytes a request body may carry. Every body on this API is a small
  /// JSON object — a handshake, a port number — so anything larger is not a
  /// peer; the server binds every interface, and `auth/start` is open by
  /// design, so a body is the one thing a stranger on the LAN can make it
  /// hold in memory.
  static const int maxBodyBytes = 64 * 1024;

  /// Most handshakes that may be waiting on their `auth/complete` at once.
  /// Each costs one map entry for [handshakeTtl]; a stranger choosing a fresh
  /// nonce per request would otherwise grow the map for as long as the
  /// Sync P2P dialog stays open.
  static const int maxPendingHandshakes = 64;
...
class PeerSyncServer {
  ...
  /// The request's JSON body, or null when it is not a JSON object — or when
  /// it runs past [PeerProtocol.maxBodyBytes], in which case the rest is left
  /// unread: dart:io then drops the connection, which is the right answer to a
  /// body no peer would send.
  Future<Map<String, dynamic>?> _body(HttpRequest request) async {
    try {
      ...
      await for (final chunk in request) {
        total += chunk.length;
        if (total > PeerProtocol.maxBodyBytes) return null;
        chunks.add(chunk);
      }
      final text = utf8.decode([for (final chunk in chunks) ...chunk]);
  ...
  Future<void> _authStart(HttpRequest request) async {
    ...
    // Full: the oldest pending handshake makes room. A real peer completes
    // within a round trip, so the one evicted is nearly always a stray.
    while (_pendingAuth.length >= PeerProtocol.maxPendingHandshakes) {
      _pendingAuth.remove(_pendingAuth.keys.first);
    }
