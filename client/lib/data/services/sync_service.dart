      ...
      for (final p in packets) {
        ...
        if (!await _ensureBlobsOnRelay(client, blobIds)) {
          _log?.add(SyncLogKind.pushBlocked,
              ...
              counter: p.counter,
              packetHash: hash,
              message: 'Not uploaded: an attachment it references is not on '
  ...
  bool _remoteWins(SyncChange change, String? latestLocalTimestamp) {
    ...
    if (ahead == Duration.zero) {
      _noteChange(SyncLogKind.lwwTie,
          ...
          message: 'Same timestamp on both sides: each device keeps its own '
              'value, so they stay different');
    } else {
  ...
  Future<_ApplyOutcome> _createTaskFromRemote(
    ...
    if (parentId == null &&
      ...
      _noteChange(SyncLogKind.parentMissing,
          level: SyncLogLevel.warning,
          message: 'Created at the root: its parent ${change.parentWorldId} '
      ...
      case 'parentId':
        if (change.value == null || change.value!.isEmpty) {
          ...
          if (parent != null) {
            ...
            _noteChange(SyncLogKind.ignored,
                level: SyncLogLevel.warning,
                message: 'Move ignored: new parent ${change.value} is not on '
                    'this device');
          }
        }
        return _ApplyOutcome.applied;
    ...
    switch (change.field) {
      ...
      case 'removed':
        ...
        if (removed == 1) {
          ...
          _noteChange(SyncLogKind.ignored,
              level: SyncLogLevel.warning,
              message: 'Time entries cannot be restored; ignored');
        }
        return _ApplyOutcome.applied;
      default:
        _noteChange(SyncLogKind.unknownField,
