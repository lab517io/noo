...
class SyncEntityChange {
  ...
  /// What changed, e.g. `title → "Project X"` or `09:00–10:30 on "Project X"`.
...
class SyncResult {
  ...
  /// of silent loss. Both are invisible in the change counts by construction:
  /// a skipped packet and a lost conflict each apply zero changes, which reads
  /// as "already up to date".
  bool get hasDiagnostics => skippedPackets.isNotEmpty || lwwSkips.isNotEmpty;
}
...
class SyncService {
  ...
  Future<SyncResult> performSync({void Function(SyncProgress)? onProgress}) async {
    ...
    if (client == null) {
      ...
      return const SyncResult(
        ...
        error: 'No sync server is configured. '
            'Use "Sync with Nearby Devices" for local sync, or add a server '
            'in Preferences → Sync.',
      );
    }
    if (_syncInProgress) {
    ...
    try {
      ...
      pullCount += await pullFromSource(
        client,
        onApplied: (total) =>
            emit(SyncStageState.active, detail: '$total applied'),
  ...
  Future<int> exchangeWithSource(PacketSource source) async {
    if (_syncInProgress) return -1;
    _syncInProgress = true;
    _beginRunDiagnostics();
    try {
      await prepareCrypto();
      // Fork check before packaging, for the same reason as in performSync.
      final catchingUp = await _checkForFork(source, await source.getVector());
  ...
  Future<bool> fetchAttachmentNow(int attachmentId) async {
    ...
    try {
      ...
      final filled = await _fetchMissingBlobs(
        client,
        budgetBytes: _noBudget,
        only: row.contentHash,
      );
      return filled > 0;
    } catch (_) {
      return false;
  ...
  /// A budget that cannot be exhausted, for a fetch the user asked for.
  static const int _noBudget = 1 << 62;
  ...
  Future<({int collected, String? blocked})> collectRemovedBlobs() async {
    ...
    return (collected: await _db.collectRemovedBlobs(), blocked: null);
  }
  ...
  ///
  /// Returns false when a blob is not on this device either: a carried packet
  /// whose attachment has not reached us yet. The caller must then stop
  /// carrying that stream at this packet, since contiguity forbids skipping.
  Future<bool> _ensureBlobsOnRelay(
      SyncApiClient client, List<String> blobIds) async {
  ...
  Future<int> pushToRelay({Map<String, int>? relayVector}) async {
    ...
    for (final entry in localVector.entries) {
      ...
      for (final p in packets) {
        ...
        if (!await _ensureBlobsOnRelay(client, blobIds)) break;
        await client.uploadPacket(
  ...
  Future<int> pullFromSource(
    ...
    while (hasMore) {
      ...
      for (final packet in result.changes) {
        ...
        if (outcome.deferred) deferred.add(packet);
      }
      ...
      while (deferred.isNotEmpty) {
        ...
        for (final packet in deferred) {
          ...
          totalApplied += outcome.applied;
          if (outcome.progressed) progressed = true;
          if (outcome.deferred) again.add(packet);
        }
  ...
  /// one corrupt blob must not wedge the stream.
  ...
  /// Apply an already-decoded packet. Returns null when applying it threw —
  /// the packet is then recorded in [skippedPackets] and the stream moves on.
  ...
  Future<int> retryOrphans({List<SyncChange>? changeSink}) async {
    ...
    await _db.purgeSyncOrphansBefore(NooDatabase.formatIso(
  ...
  /// in the change counts, and the two causes worth catching — a clock that
  /// runs ahead on this device, and the exact tie where both sides skip each
  /// other and stay diverged — look exactly like a change that never arrived.
  bool _remoteWins(SyncChange change, String? latestLocalTimestamp) {
    ...
    if (remote.isAfter(local)) return true;
  ...
  Future<_ApplyOutcome> _applyTaskChange(
    ...
    if (!_remoteWins(change, latest?.timestamp)) {
  ...
  Future<void> _noteEditConflict(
    ...
    // back with creation flags; they are not concurrent edits.
    // Full-state re-announces (migration, fork recovery) echo stale values
  ...
  Future<_ApplyOutcome> _applyFileChange(SyncChange change) async {
    ...
    if (!_remoteWins(change, latest?.timestamp)) return _ApplyOutcome.skipped;
  ...
  Future<List<SyncEntityChange>> _summarizeChanges(
    ...
    for (final key in order) {
      final summary = await _summarizeEntity(groups[key]!);
  ...
  Future<SyncEntityChange?> _summarizeEntity(List<SyncChange> group) async {
    switch (group.first.entityType) {
      case 'task':
        return _summarizeTask(group);
      case 'file':
        return _summarizeFile(group);
      case 'timeline':
        return _summarizeTimeline(group);
      default:
        return null;
    }
  }

  Future<SyncEntityChange> _summarizeTask(List<SyncChange> group) async {
    ...
    if (kind == SyncEntityChangeKind.updated) {
      ...
      if (_lastField(group, 'parentId') != null) parts.add('moved');
  ...
  Future<SyncEntityChange> _summarizeFile(List<SyncChange> group) async {
  ...
  Future<SyncEntityChange> _summarizeTimeline(List<SyncChange> group) async {
    ...
    final taskName = (task?.title.isNotEmpty ?? false) ? task!.title : null;
    ...
    // Compose the detail: time range and/or the owning task.
    String? detail;
    if (kind != SyncEntityChangeKind.removed) {
      final parts = <String>[];
      if (range != null) parts.add(range);
      if (taskName != null) parts.add('on "$taskName"');
      if (parts.isNotEmpty) detail = parts.join(' ');
