...
class AttachmentMediaServer {
  ...
  ///
  /// A [stop] that arrives while the bind is still pending wins: the socket is
  /// closed as soon as it comes back rather than left listening with no owner
  /// — which is what happens when the provider that started the server is
  /// disposed by a database swap before `bind` returns.
  ...
  /// Moved on by every [start] and [stop], so a start can tell whether a stop
  /// overtook it while it was binding.
  int _generation = 0;
