...
class HistoryService {
  ...
  ///
  /// A patch that does not apply — a chain with a row missing, so the hunk's
  /// context is not in the running value — yields [unreconstructable] for
  /// that row and every one after it, since each later patch was made against
  /// text this replay no longer has. Showing dmp's best effort instead would
  /// present a splice of two versions as the stored value.
  static List<({String? oldValue, String? newValue})> reconstructTaskFieldChain(
    ...
    for (final row in rows) {
      ...
      if (stored == null) {
        current = null;
        broken = false; // a deletion needs no base; the chain is whole again
        ...
        current = broken ? null : DiffUtils.applyDiff(base, stored);
        if (current == null) {
          broken = true;
          current = unreconstructable;
        }
      } else {
  ...
  /// Placeholder [reconstructTaskFieldChain] returns for a row whose patch
  /// could not be replayed onto the value before it.
  static const String unreconstructable = '(history could not be reconstructed)';
