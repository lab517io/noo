...
class FakeVoiceMemoEngine implements VoiceMemoEngine {
  ...
  /// When set, [start] waits on it before opening — a permission prompt the
  /// user has not answered yet, or a device that is slow to open — so a test
  /// can cancel from inside that window.
  Completer<void>? startGate;

  /// Same for [stop]: the encoder tail, during which the recorder is no longer
  /// recording but the clip has not come back yet.
...
void main() {
  ...
  group('VoiceMemoRecorder', () {
    ...
    test('cancel while the device is opening closes it again', () async {
      final engine = FakeVoiceMemoEngine()..startGate = Completer<void>();
      final recorder = recorderFor(engine);

      // The permission prompt is up, or the device is slow to open. Nothing is
      // recording yet, which used to make cancel a no-op — and the start then
      // carried on and left the microphone live with nobody holding it.
