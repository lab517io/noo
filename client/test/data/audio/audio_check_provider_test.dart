...
void main() {
  ...
  group('AudioCheckNotifier', () {
    ...
    test('cancel while the device is opening leaves no recorder behind',
        () async {
      ...
      // Not stuck: the next Test starts and stops like any other.
    ...
    test('cancel during the encoder tail never plays the clip', () async {
      engine.stopGate = Completer<void>();
      final container = await makeContainer();
      final notifier = container.read(audioCheckProvider.notifier);

      await notifier.start();
      // Stop pressed: the recorder is no longer recording but the clip has
      // not come back yet. Cancel here has nothing to cancel and nothing to
      // stop, so the run itself has to notice it was abandoned.
      final finishing = notifier.finish();
      await notifier.cancel();
