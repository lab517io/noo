...
void main() {
  ...
  group('diff-field history storage', () {
    ...
    test('a patch that does not apply is reported, not spliced in (DEEPSEEK L3)',
        () async {
      final id = await db.createTask(worldId: 'w-1', title: 'hello world');
      // A patch made against text this chain never held — what a row missing
      // from the middle of a chain looks like on replay.
      await db.insertTaskHistory(
        taskId: id,
        field: 'title',
        oldValue: '',
        newValue: DiffUtils.computeDiff(
            'completely different base text', 'completely different base')!,
      );
      await db.updateTask(id, title: 'hello there');

      final rows = await db.getTaskHistoryForField(id, 'title');
      final pairs = HistoryService.reconstructTaskFieldChain(rows);
      expect(pairs[0].newValue, 'hello world');
      expect(pairs[1].newValue, HistoryService.unreconstructable);
      // Later patches were made against text the replay no longer has.
      expect(pairs[2].newValue, HistoryService.unreconstructable);
    });
