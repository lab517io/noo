...
void main() {
  ...
  group('providers with no server configured', () {
    ...
    test('LAN peer sync is available, but not listening', () {
      final coordinator = container.read(lanSyncCoordinatorProvider);
      expect(coordinator, isNotNull);
      expect(coordinator!.isRunning, isFalse,
          reason: 'peer server and discovery only run while Sync P2P is open');
    });
  });
  ...
  group('SyncService without a relay', () {
    ...
    test('performSync reports the missing server instead of throwing',
      ...
      expect(result.error, contains('Sync P2P'));
    });
