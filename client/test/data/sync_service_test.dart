...
class _FakeServer {
  ...
  /// The relay's blob size limit (`NOO_MAX_BLOB_SIZE`), when a test sets one:
  /// a larger upload is refused with 413 the way the real relay refuses it.
  int? maxBlobBytes;

  /// Every blob PUT attempted, refused ones included.
  final List<String> blobPuts = [];
...
class _FakeApiClient extends SyncApiClient {
  ...
  Future<void> putBlob(String blobId, Uint8List encrypted) async {
    server.blobPuts.add(blobId);
    final limit = server.maxBlobBytes;
    if (limit != null && encrypted.length > limit) {
      throw SyncApiException(
          'API error: {"error":"Blob exceeds maximum size of $limit bytes"}',
          413);
    }
...
void main() {
  ...
  group('SyncService round trip', () {
    ...
    // The report names a node by its title, and a title alone does not say
    // which "Notes" it was or where a moved one went.
    test('the report gives a changed node its path, as it is after the run',
        () async {
      final work = await _createTask(dbA, WorldId.create(), 'Work');
      final q3 = await dbA.createTask(
          worldId: WorldId.create().value, parentId: work, title: 'Q3');
      final home = await _createTask(dbA, WorldId.create(), 'Home');
      final wid = WorldId.create();
      final id = await dbA.createTask(
          worldId: wid.value, parentId: home, title: 'Budget');
      await syncA.performSync();
      await syncB.performSync();

      await dbA.updateTask(id, title: 'Budget review', parentId: q3);
      final push = await syncA.performSync();
      final pull = await syncB.performSync();

      for (final result in [push, pull]) {
        final change = result.applied.isEmpty
            ? result.pushed.single
            : result.applied.single;
        expect(change.label, 'Node "Budget review"');
        expect(change.detail, contains('moved'));
        expect(change.path, ['Work', 'Q3']);
      }
    });

    test('a node moved to the top level says so; one already there says '
        'nothing', () async {
      final parent = await _createTask(dbA, WorldId.create(), 'Inbox');
      final moved = await dbA.createTask(
          worldId: WorldId.create().value, parentId: parent, title: 'Moved');
      final renamed = await _createTask(dbA, WorldId.create(), 'Top');
      await syncA.performSync();
      await syncB.performSync();

      await dbA.updateTask(moved, clearParent: true);
      await dbA.updateTask(renamed, title: 'Top level');
      await syncA.performSync();
      final pull = await syncB.performSync();

      final byLabel = {for (final c in pull.applied) c.label: c};
      expect(byLabel['Node "Moved"']!.path, isEmpty);
      expect(byLabel['Node "Top level"']!.path, isNull);
    });

    test('a file and a time chunk are placed under the node they belong to',
        () async {
      final work = await _createTask(dbA, WorldId.create(), 'Work');
      final task = await dbA.createTask(
          worldId: WorldId.create().value, parentId: work, title: 'Budget');
      ...
      final file = pull.applied.singleWhere((c) => c.entityType == 'file');
      final time = pull.applied.singleWhere((c) => c.entityType == 'timeline');
      expect(file.path, ['Work', 'Budget']);
      expect(time.path, ['Work', 'Budget']);
      // The node is the end of the path now, not repeated in the detail.
      expect(time.detail, isNot(contains('Budget')));
    });
  ...
  group('sync log', () {
    ...
    test('an exact timestamp tie is a warning carrying both timestamps',
      ...
      expect(tie.message, contains('the incoming one'));
      final warnings = await dbB.getSyncLogEvents(minLevel: 1);
      expect(warnings.map((e) => e.kind), contains(SyncLogKind.lwwTie));
      // The tie was broken, not left standing: the greater value is now here.
      expect((await dbB.getTaskByWorldId(wid.value))!.title, 'Theirs');
    });
  ...
  group('convergence', () {
    ...
    /// A change to [wid]'s title from device-c stamped exactly [timestamp].
    Future<void> tieFrom(NooDatabase db, WorldId wid, String value,
            String timestamp) =>
        _injectPacket(server, db, 'device-c', [
          SyncChange(
            entityType: 'task',
            worldId: wid.value,
            field: 'title',
    ...
    test('an exact tie goes to the greater value: the incoming side', () async {
      final wid = WorldId.create();
      final id = await _createTask(dbB, wid, 'Mine');
      final local = await dbB.getLatestTaskHistoryForField(id, 'title');

      await tieFrom(dbB, wid, 'Theirs', local!.timestamp);
      final res = await syncB.performSync();

      expect(res.success, isTrue);
      expect(res.changesApplied, 1);
      expect(res.lwwSkips, isEmpty, reason: 'it was applied, not lost');
      expect((await dbB.getTaskByWorldId(wid.value))!.title, 'Theirs');
    });

    test('an exact tie goes to the greater value: the side already here',
        () async {
      final wid = WorldId.create();
      final id = await _createTask(dbB, wid, 'Zulu');
      final local = await dbB.getLatestTaskHistoryForField(id, 'title');

      // The mirror of the test above — what device-c sees when it receives
      // B's 'Zulu' against its own 'Alpha' is what B sees here. One side
      // applies, the other keeps, and both end on 'Zulu'.
      await tieFrom(dbB, wid, 'Alpha', local!.timestamp);
      final res = await syncB.performSync();

      expect(res.success, isTrue);
      expect(res.changesApplied, 0);
      expect(res.lwwSkips, hasLength(1));
      expect((await dbB.getTaskByWorldId(wid.value))!.title, 'Zulu');
    });

    test('a tie on the same value changes nothing and reports no loss',
        () async {
      final wid = WorldId.create();
      final id = await _createTask(dbB, wid, 'Same');
      final local = await dbB.getLatestTaskHistoryForField(id, 'title');

      await tieFrom(dbB, wid, 'Same', local!.timestamp);
      final res = await syncB.performSync();

      expect(res.changesApplied, 0);
      expect(res.lwwSkips, isEmpty);
      expect((await dbB.getTaskByWorldId(wid.value))!.title, 'Same');
    });

    test('a child served before its parent is moved under it once the parent '
        'arrives', () async {
      // A creates the parent; B (synced) creates a child under it. A fresh
      // device is served B's stream first, so the child's creation names a
      // parent it has never seen.
      final parent = WorldId.create();
      await _createTask(dbA, parent, 'Parent');
      await syncA.performSync();
      await syncB.performSync();
      final parentOnB = (await dbB.getTaskByWorldId(parent.value))!;
      final child = WorldId.create();
      await dbB.createTask(
          worldId: child.value, title: 'Child', parentId: parentOnB.id);
      await syncB.performSync();
      ...
      try {
        ...
        final parentOnC = (await dbC.getTaskByWorldId(parent.value))!;
        final childOnC = (await dbC.getTaskByWorldId(child.value))!;
        expect(childOnC.parentId, parentOnC.id,
            reason: 'the provisional root placement was corrected by retry');
        expect(await dbC.getAllSyncOrphans(), isEmpty);
      } finally {
        await dbC.close();
      }
    });

    test('a move under a parent not yet known is retried, not dropped',
        () async {
      // T exists everywhere. B creates P; A, having pulled P, moves T under
      // it. A fresh device served A's stream before B's sees the move first.
      final t = WorldId.create();
      final tId = await _createTask(dbA, t, 'Moved');
      await syncA.performSync();
      await syncB.performSync();

      final p = WorldId.create();
      await _createTask(dbB, p, 'New home');
      await syncB.performSync();
      await syncA.performSync();
      final pOnA = (await dbA.getTaskByWorldId(p.value))!;
      await dbA.updateTask(tId, parentId: pOnA.id);
      await syncA.performSync();

      server.orderHint = ['device-a', 'device-b'];
      final dbC = NooDatabase.memory();
      final syncC = _service(dbC, server, 'device-c');
      try {
        final res = await syncC.performSync();
        ...
        final pOnC = (await dbC.getTaskByWorldId(p.value))!;
        final tOnC = (await dbC.getTaskByWorldId(t.value))!;
        expect(tOnC.parentId, pOnC.id);
        expect(await dbC.getAllSyncOrphans(), isEmpty);
      } finally {
        await dbC.close();
      }
    ...
    test('a restored time entry is restored on the other device too',
        () async {
      final wid = WorldId.create();
      final taskId = await _createTask(dbA, wid, 'Timed');
      final entry = WorldId.create();
      final entryId = await dbA.createTimeRecord(
        taskId: taskId,
        worldId: entry.value,
        startTime: '2026-09-25T08:00:00.000000Z',
        endTime: '2026-09-25T09:00:00.000000Z',
      );
      await syncA.performSync();
      await syncB.performSync();
      expect((await dbB.getTimeRecordByWorldId(entry.value))!.removed, 0);

      await dbA.deleteTimeRecord(entryId);
      await syncA.performSync();
      await syncB.performSync();
      expect((await dbB.getTimeRecordByWorldId(entry.value))!.removed, 1);

      await dbA.undeleteTimeRecord(entryId);
      await syncA.performSync();
      final res = await syncB.performSync();
      expect(res.changesApplied, 1);
      expect((await dbB.getTimeRecordByWorldId(entry.value))!.removed, 0);
    ...
    test('an attachment the relay refuses is named in the result and not '
        'uploaded twice', () async {
      server.maxBlobBytes = 1024;
      final wid = WorldId.create();
      final taskId = await _createTask(dbA, wid, 'Heavy');
      await dbA.createAttachment(
        taskId: taskId,
        worldId: WorldId.create().value,
        filename: 'huge.bin',
        content: Uint8List.fromList(List.filled(4096, 1)),
      );

      final first = await syncA.performSync();
      expect(first.success, isTrue, reason: 'the pull still ran');
      expect(first.warning, contains('"huge.bin"'));
      expect(first.warning, contains('4 KB'));
      expect(first.warning, contains('limit is 1 KB'));
      expect(first.hasDiagnostics, isTrue);
      expect(server.totalPackets, 0,
          reason: 'the packet naming the blob cannot be stored without it');
      expect(server.blobPuts, hasLength(1));

      // The relay's answer will not change: the next run reports again
      // without sending the bytes again.
      final again = await syncA.performSync();
      expect(again.warning, contains('"huge.bin"'));
      expect(server.blobPuts, hasLength(1));
    });

    test('only a malformed packet is written off; a device failure is retried',
        () {
      expect(SyncService.isPacketFault(const FormatException('bad')), isTrue);
      expect(SyncService.isPacketFault(ArgumentError('bad')), isTrue);
      expect(SyncService.isPacketFault(RangeError.value(9)), isTrue);
      expect(SyncService.isPacketFault(StateError('database closed')), isFalse);
      expect(SyncService.isPacketFault(Exception('disk I/O error')), isFalse);
    });
  });
}
