...
void main() {
  ...
  test('a v11 database upgrades all the way in one open', () async {
    ...
    // opened by one that is at 14. Every migration has to land, and the data
    ...
    // v14: the sync log tables exist and work.
    final run = await db.startSyncLogRun(trigger: 'relay');
    await db.insertSyncLogEvents([
      SyncLogEventsCompanion.insert(
          runId: run, seq: 0, at: NooDatabase.nowIso(), kind: 'note'),
    ]);
    expect(await db.getSyncLogEvents(runId: run), hasLength(1));
