...
void main() {
  ...
  testWidgets('Scrolling the Appearance tab reaches the last group', (
    ...
    // Three font groups now — editor, tree, interface — so the drag has to
    // carry further than it once did, and each group has its own Preview.
    await tester.drag(find.text('Preview:').first, const Offset(0, -700));
    await tester.pumpAndSettle();
  ...
  testWidgets('Cancel also restores font settings', (tester) async {
    ...
    // The first Bold and the first stepper are the editor font's; the tree
    // font below has its own.
    await tester.tap(find.widgetWithText(ClassicCheckbox, 'Bold').first);
  ...
  /// Ticks the editor's Monospace-fonts-only box, which lives on Behavior.
  ///
  /// The label is tapped rather than the widget: this checkbox carries a hint
  /// under it, so its centre falls between the two and misses the row.
  Future<void> tickMonospaceOnly(WidgetTester tester) async {
    await tester.tap(find.text('Behavior'));
    await tester.pumpAndSettle();
    await tester.tap(find.text('Monospace fonts only'));
    await tester.pumpAndSettle();
  }

  testWidgets('Monospace fonts only leaves the content font alone',
      (tester) async {
    final container = await showDialogFrom(tester);
    final before = container.read(settingsProvider);

    await tickMonospaceOnly(tester);

    expect(container.read(settingsProvider).monospaceFontsOnly, isTrue);
    expect(container.read(settingsProvider).fontFamily, before.fontFamily,
        reason: 'the content font dresses the tree too, and the tree has no '
            'columns to line up');
  });

  testWidgets('Cancel rolls the editor font preference back', (tester) async {
    final container = await showDialogFrom(tester);
    final before = container.read(settingsProvider);
    ...
    final after = container.read(settingsProvider);
    expect(after.monospaceFontsOnly, before.monospaceFontsOnly);
    expect(after.fontFamily, before.fontFamily);
  });
}
