...
void main() {
  ...
  test('a selection dragged right-to-left is replaced, not kept', () {
    final controller = QuillController.basic();
    addTearDown(controller.dispose);

    controller.document.insert(0, 'abc');
    // Extent before base: what a mouse drag from the end to the start gives.
    controller.updateSelection(
      const TextSelection(baseOffset: 3, extentOffset: 1),
      ChangeSource.local,
    );

    insertImageEmbed(controller, attachmentUri('w-2'));

    final ops = controller.document.toDelta().toJson();
    expect(ops.first['insert'], 'a');
    expect(ops[1]['insert'], {'image': attachmentUri('w-2')});
    expect(controller.document.toPlainText(), isNot(contains('bc')),
        reason: 'the selected text goes, whichever way it was selected');
    expect(controller.selection.baseOffset, 2);
  });

  test('replacing the bytes behind a worldId makes a new provider unequal',
      () {
    Future<Uint8List?> loader(String _) async => null;
    final before = AttachmentImage('w-3', loader: loader);
    expect(AttachmentImage('w-3', loader: loader), before,
        reason: 'the loader closure never takes part in equality');

    AttachmentImage.evictWorldId('w-3');

    final after = AttachmentImage('w-3', loader: loader);
    expect(after, isNot(before),
        reason: 'a mounted Image only re-resolves for a provider that '
            'compares unequal, and the worldId is the same across a replace');
    expect(after.hashCode, isNot(before.hashCode));
    expect(AttachmentImage('w-4', loader: loader),
        AttachmentImage('w-4', loader: loader),
        reason: 'other worldIds are untouched');
  });
